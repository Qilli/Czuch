...
namespace Czuch
{
	...
	void Scene::FillRenderContexts(Camera* cam, Renderer* renderer, int width, int height, RenderContextFillParams& fillParams)
	{
		...
		for (auto entity : renderableView)
		{
			...
			auto currentMaterial = meshRenderer.GetOverrideMaterial();
			auto currentMaterialInstance = renderer->GetDevice()->AccessMaterialInstance(currentMaterial);
			auto material = fillParams.forceMaterialForAll ? (fillParams.ignoreTransparent&&currentMaterialInstance->IsTransparent() ? meshRenderer.GetOverrideMaterial(): fillParams.forcedMaterial) : meshRenderer.GetOverrideMaterial();
